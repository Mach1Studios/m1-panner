...
CONTROL_FILE_PATTERN="M1SpatialSystem_M1Panner_PID*_PTR*_T*_Control.mem"
...
echo "  - Audio data: $MEMORY_FILE_PATTERN"
echo "  - Control data: $CONTROL_FILE_PATTERN"
echo ""
...
monitor_memory_files() {
    ...
    # Look for audio and control memory files in all directories
    audio_files=()
    control_files=()
    ...
    for temp_dir in "${TEMP_DIRS[@]}"; do
        if [ -d "$temp_dir" ]; then
            ...
            control_files+=($(ls "$temp_dir"/$CONTROL_FILE_PATTERN 2>/dev/null || true))
        fi
    done
    
    if [ ${#audio_files[@]} -eq 0 ] && [ ${#control_files[@]} -eq 0 ]; then
    ...
    echo "Found ${#audio_files[@]} audio memory file(s) and ${#control_files[@]} control memory file(s)"
...
int main(int argc, char* argv[]) {
    ...
    if (header->hasData && header->dataSize > 8) {
        uint8_t* dataBuffer = (uint8_t*)mapped + sizeof(struct SharedMemoryHeader);
        uint32_t* channelsPtr = (uint32_t*)dataBuffer;
        uint32_t* samplesPtr = (uint32_t*)(dataBuffer + 4);
        
        printf("\n=== Audio Data Info ===\n");
        printf("Audio Channels: %u\n", *channelsPtr);
        printf("Audio Samples: %u\n", *samplesPtr);
        
        if (*channelsPtr > 0 && *samplesPtr > 0 && *channelsPtr <= 8 && *samplesPtr <= 8192) {
            float* audioData = (float*)(dataBuffer + 8);
            printf("First few audio samples:\n");
            for (int i = 0; i < 10 && i < (*samplesPtr * *channelsPtr); i++) {
                printf("  Sample %d: %.6f\n", i, audioData[i]);
            }
            
            // Output samples for waveform visualization (first 200 samples, channel 0 only)
            printf("\nWAVEFORM_DATA_START\n");
            int max_samples = (*samplesPtr < 200) ? *samplesPtr : 200;
            for (int i = 0; i < max_samples; i++) {
                // Get sample from first channel (interleaved format: ch0, ch1, ch0, ch1, ...)
                float sample = audioData[i * (*channelsPtr)];
                printf("%.6f\n", sample);
            }
...
# Function to read control data
read_control_data() {
    local file=$1
    echo "Reading control data from: $file"
    
    # Control data should be JSON string, try to read it
    if [ -f "$file" ]; then
        # Skip the header and try to read the string data
        local header_size=88  # Approximate header size
        if command -v tail >/dev/null 2>&1; then
            echo "Control data content:"
            tail -c +$header_size "$file" 2>/dev/null | head -c 500 | tr -d '\0' | sed 's/^/  /'
        else
            echo "Raw control file content (first 200 bytes):"
            hexdump -C "$file" | head -12 | sed 's/^/  /'
        fi
    fi
    echo ""
}
    ...
    if monitor_memory_files; then
        ...
        if [ $((iteration % 3)) -eq 0 ]; then
            ...
            if [ ${#audio_files[@]} -gt 0 ]; then
                echo "Analyzing most recent audio file..."
                display_file_info "${audio_files[0]}" "AUDIO"
                read_audio_data "${audio_files[0]}"
            fi
            
            # Process first control file for detailed analysis
            if [ ${#control_files[@]} -gt 0 ]; then
                echo "Analyzing most recent control file..."
                display_file_info "${control_files[0]}" "CONTROL"
                read_control_data "${control_files[0]}"
            fi
        else
            echo "Memory sharing is active! Audio data should be changing if audio is playing."
            echo "Found ${#audio_files[@]} audio file(s) and ${#control_files[@]} control file(s)"
