...
#include <atomic>
#include <memory>
#include <vector> // Added for getActiveUsers
...
public:
    ...
    /**
     * @brief Get information about active instances (for debugging/single-instance checking)
     * @return Vector of active instance identifiers
     */
     ...
     * @return true if any M1-Panner instance is using the service
     */
    bool hasActiveM1PannerInstances() const
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        for (const auto& instance : m_activeInstances) {
            if (instance.find("M1SpatialSystem_M1Panner") != std::string::npos) {
                return true;
            }
        }
        return false;
