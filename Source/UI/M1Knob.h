...
public:
    void internalDraw(Murka& m)
    {
        ...
        {
            m.prepare<murka::Label>({ 0, shape.size.x * 0.8 / width + 10, shape.size.x / width, shape.size.y * 0.5 })
        ...
        if ((mouseDownPressed(0)) && (isInside) && (mousePosition().y < labelPositionY) && (!draggingNow) && (enabled))
        {
            draggingNow = true;
            cursorHide();
        }
